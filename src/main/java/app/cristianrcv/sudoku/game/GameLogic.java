...
import app.cristianrcv.sudoku.game.constants.Rows;
...
class GameLogic {
     ...
     * - [0, 0] == [3, 4] allowed
     */
    static GameState checkForCompletion(int[][] grid) {
     ...
     * @return
     */
    static boolean tilesAreNotFilled(int[][] grid) {
     ...
     * @return
     */
    static boolean squaresAreInvalid(int[][] grid) {
        //top three squares
    ...
    static boolean rowOfSquaresIsInvalid(Rows value, int[][] grid) {
        switch (value) {
            case TOP:
                //x FIRST = 0
    ...
    static boolean squareIsInvalid(int yIndex, int xIndex, int[][] grid) {
    ...
    static boolean columnsAreInvalid(int[][] grid) {
        for (int xIndex = 0; xIndex < SudokuGame.GRID_BOUNDARY; xIndex++) {
    ...
    static boolean rowsAreInvalid(int[][] grid) {
        for (int yIndex = 0; yIndex < SudokuGame.GRID_BOUNDARY; yIndex++) {
            List<Integer> row = new ArrayList<>();
            for (int xIndex = 0; xIndex < SudokuGame.GRID_BOUNDARY; xIndex++) {
                row.add(grid[xIndex][yIndex]);
